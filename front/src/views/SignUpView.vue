  ...
  <div>
    <div class="row">
      <b-button
        ...
        class="bouton col-md-1 d-flex justify-content-center"
        @click="retourDashboard()"
        >Retour</b-button
      >
      <h1 class="mb-5 mt-5 d-flex justify-content-center" v-if="isModifying">
        Modifer profil
      </h1>
      <h1 class="mb-5 mt-5 d-flex justify-content-center" v-else>
        Créer un compte
      </h1>
      <div class="col-md-9">
        <b-input-group class="mb-4">
          ...
          <b-form-input
            ...
            v-model="nom"
            v-bind:class="{
              'is-invalid': isTryingToSave && (isEmptyNom || isNomCorrect),
            }"
          ></b-form-input>
        </b-input-group>
        <div
          ...
          role="alert"
          v-if="isTryingToSave && isEmptyNom"
        >
          Champ non remplit
        </div>
        <div
          class="alert alert-danger"
          role="alert"
          v-if="isTryingToSave && isNomCorrect"
        >
          Nom non valide
        </div>
        <b-input-group class="mb-4">
          ...
          <b-form-input
            ...
            v-bind:class="{
              'is-invalid':
                isTryingToSave && (isEmptyPrenom || isPrenomCorrect),
            }"
          ></b-form-input>
        </b-input-group>
        <div
          class="alert alert-danger"
          role="alert"
          v-if="isTryingToSave && isEmptyPrenom"
        >
          Champ non remplit
        </div>
        <div
          class="alert alert-danger"
          role="alert"
          v-if="isTryingToSave && isPrenomCorrect"
        >
          Prénom non valide
        </div>
        <b-input-group class="mb-4">
          ...
          <b-form-input
            ...
            v-bind:class="{
              'is-invalid':
                isTryingToSave && (isEmptyPseudo || isPseudoTooSmall),
            }"
          ></b-form-input>
        </b-input-group>
        <div
          class="alert alert-danger"
          role="alert"
          v-if="isTryingToSave && isEmptyPseudo"
        >
          Champ non remplit
        </div>
        <div
          class="alert alert-danger"
          role="alert"
          v-if="isTryingToSave && isPseudoTooSmall"
        >
          Pesudo trop court
        </div>
        <b-input-group class="mb-4">
          ...
          <b-form-input
            ...
            v-model="mail"
            v-bind:class="{
              'is-invalid': isTryingToSave && (isEmptyMail || isMailCorrect),
            }"
          ></b-form-input>
        </b-input-group>
        <div
          class="alert alert-danger"
          role="alert"
          v-if="isTryingToSave && isEmptyMail"
        >
          Champ non remplit
        </div>
        <div
          class="alert alert-danger"
          role="alert"
          v-if="isTryingToSave && isMailCorrect"
        >
          E-mail incorrect
        </div>
        <b-input-group class="mb-4">
          ...
          <b-form-input
            ...
            v-model="mdp"
            v-bind:class="{
              'is-invalid': isTryingToSave && (isEmptyMdp || isMdpCorrect),
            }"
          ></b-form-input
          ><br />
          <i
            class="fas fa-eye position-absolute top-50 end-0 translate-middle"
            @click="afficherMdp()"
            style="z-index: 99"
          ></i>
        </b-input-group>
        <div
          class="alert alert-danger"
          role="alert"
          v-if="isTryingToSave && isEmptyMdp"
        >
          Champ non remplit
        </div>
        <div
          class="alert alert-danger"
          role="alert"
          v-if="isTryingToSave && isMdpCorrect"
        >
          Le mot de passe doit contenir 10 charactère avec au moins 1 majuscule
          et 1 chiffre
        </div>
        <b-button
  ...
  computed: {
    ...
    isFullOk() {
      return !(
        ...
        this.isNomCorrect &&
        this.isEmptyPrenom &&
        this.isPrenomCorrect &&
        this.isEmptyPseudo &&
        this.isPseudoTooSmall &&
        this.isEmptyMail &&
        this.isMailCorrect &&
        this.isEmptyMdp &&
        this.isMdpCorrect
